<template>
   <div class="bg-white w-full h-auto  p-4" >
        ...
        <div class="w-full flex items-center justify-left border-b border-b-gray-300 pb-4 relative">
            ...
            <span  class="font-semibold text-gray-800">Zonas</span>
        </div>
        ...
        <div class="w-full h-full mt-4 border border-gray-200">
            ...
            <div class="card pb-40">
                    <DataTable v-model:filters="filters"  :value="zonas" paginator :rows="5" :rowsPerPageOptions="[5, 10, 20, 50]" tableStyle="min-width: 50rem">
                        <template #header>
                        ...
                        <template #body="{data}">
                            ...
                            <Button @click="eliminar(data.id)" icon="pi pi-trash" severity="danger" text rounded aria-label="Cancel" />
            ...
            <Dialog v-model:visible="visible" modal header="Zona"  class="w-11/12 lg:w-1/2" >
...
export default {
  ...
  components: {
      ModalUser: defineAsyncComponent(() => import('../../modal/Modal-Zona.vue'))
    },
  methods:{
    loadData() {
        this.loadZonas();
    ...
    ...mapActions('inventario', ['loadZonas', 'deleteZona']),
  ...
  computed: {
        ...mapGetters('inventario', ['getZonas']),
